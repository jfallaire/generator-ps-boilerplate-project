...
class Setting(SupportsInt, SupportsFloat, Generic[T]):
    ...
    """
    def __init__(self, key: str, fallback: ConfigValue = None, alternate_keys: Collection[str] = None) -> None:
        """ Initializes a setting. """
        self._key: str = key
        self._alternate_keys: Collection[str] = alternate_keys or tuple()
        self.fallback = fallback
        if fallback is not None:
            self.cast_and_validate(fallback)  # validate fallback value beforehand
...
def mock_config_value(setting: Setting, value: Optional[ConfigValue]) -> Generator[None, None, None]:
    """ Mocks a setting value during a block of code so that environment variables are ignored. """
